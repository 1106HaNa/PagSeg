...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, ignore_residual=True, model_type='ClearCLIP'):
        output = 0 

        for i, resblock in enumerate(self.resblocks):
            if i == self.layers - 1:
                if ignore_residual:
                    attn_output = resblock.custom_attn(resblock.attn, resblock.ln_1(x), model_type=model_type)
                    output += attn_output
                    x = resblock(x)
                else:
                    attn_output = resblock.custom_attn(resblock.attn, resblock.ln_1(x), model_type=model_type)
                    x_out = x + attn_output
                    x_out = x_out + resblock.mlp(resblock.ln_2(x_out))
                    output += x_out
                    x = resblock(x)
            else:
                x = resblock(x)
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False):
        ...
        x = self.transformer(x, ignore_residual=True, model_type='ClearCLIP')
